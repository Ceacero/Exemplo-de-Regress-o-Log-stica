...
# manipulação de dados e broom para predição a partir de modelos
...
# de testes de hipóteses de Shapiro-Wilk.
...
TukeyHSD(modelo, ordered = TRUE)

plot(TukeyHSD(modelo, ordered = TRUE))

# Veja que nenhum dos intervalos de confiança incluem o 0. Logo, há evidências
...
modelo2 = glm(irisAlternativa$Species_dummy ~ irisAlternativa$Petal.Length, 
              family = "binomial")

# Calculando, pelo modelo, qual a probabilidade de cada flor da base de dados
# ser da espécie virginica

irisAlternativa$probabilidade <- round(predict(modelo2, type = "response"), 2)

# Com isso, ajustamos o modelo de regressão logística útil para prever,
